...
import { Loader2, Wrench, Search, Plus, Sparkles, Users, ShieldAlert, ShoppingCart, ExternalLink, PackageCheck, Download, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function Marketplace() {
  ...
  // 1. Store the user's active projects to check for already-owned templates!
  ...
  // 2. Fetch both public marketplace templates AND your personal active projects simultaneously
  const fetchMarketplaceData = async () => {
    try {
      const [publicRes, myProjectsRes] = await Promise.all([
        api.get('/projects/public'),
        api.get('/projects'), // Fetch user's own projects
      ]);

      if (publicRes.data.status === 'success') {
        setTemplates(publicRes.data.data);
      }
      if (myProjectsRes.data.status === 'success') {
  ...
  const handleCloneWorkspace = async (template: PublicProject) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        // Re-fetch to update the owned status instantly
        fetchMarketplaceData();
      }
    } catch (err) {
        ...
        {filteredTemplates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTemplates.map((template) => {
              ...
              // 3. Check if the user already has this project in their own active workspaces!
              const matchingOwnedProject = myProjects.find(
                (p) => (p.clonedFrom && p.clonedFrom === template._id) || p.projectName === template.projectName
              );
              const isAlreadyOwned = Boolean(matchingOwnedProject);
                ...
                >
                  ...
                  <div className="border-t border-stone-200/50 dark:border-stone-800/40 pt-4 flex items-center justify-end gap-2">
                    {userRole === 'admin' ? (
                      ...
                      /* 4. ALREADY PURCHASED / OWNED STATE! */
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-xl inline-flex items-center gap-1 select-none">
                        ...
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={() => router.push(`/dashboard/project/${matchingOwnedProject._id}`)}
                          className="flex items-center gap-1 !px-3 !py-1 text-xs shadow-sm"
                        >
